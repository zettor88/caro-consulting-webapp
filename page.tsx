...
export default function DiagnosticoPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            // First save the user info to the leads table/profiles table if needed
            let currentUserId = userAuth?.id;

            if (!currentUserId && leadData.email) {
                // Upsert anonymous user into 'usuarios' to capture lead info
                ...
                if (!leadError && leadUser) {
                    ...
                    dataToSave.usuario_id = currentUserId;
                } else {
                    console.log("Could not save lead profile separately", leadError);
                    // It's okay, we can try to save it in another way or maybe the email already exists
                }
            }
            ...
            // En background: Se puede llamar API route para generar PDF y Email
            // await fetch('/api/generate-diagnostic', { method: 'POST', body: JSON.stringify({...}) });
